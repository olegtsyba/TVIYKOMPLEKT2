...
//      (див. ACTIVE_REMINDER_STATUS_IDS) — Виняток №3 за колонкою, без
//      відкриття картки взагалі.
...
3. Виняток "уже в воронці" (з'явлення автоматичного повідомлення KeyCRM Bot з текстом на кшталт "Нагадування 2" чи "Знижка -10%" у переписці) обробляється ОКРЕМО поза цим викликом (за колонкою картки або за наявністю відправника "KeyCRM Bot" у діалозі) — якщо ти бачиш повідомлення від "KeyCRM Bot" у переписці нижче, це вже мало бути відфільтровано раніше; якщо все ж бачиш — постав exception="no_purchase_intent" з поясненням у rationale, що вже є бот-повідомлення.
  ...
  try {
    ...
    for (let i = 0; i < candidates.length; i++) {
      ...
      try {
        ...
        const hasBotMessage = dialog.messages.some((m) => normalizeCI(m.sender) === 'keycrm bot');
        if (hasBotMessage) {
          results.push({
            leadId, customerName, statusId: l.status_id, statusTitle,
            verdict: 'skip', exception: '3_already_in_funnel_transcript',
            recommendedAction: 'Завершити діалог (в переписці вже є автоматичне повідомлення KeyCRM Bot)',
            confidence: 'high',
            rationale: 'У переписці знайдено повідомлення від "KeyCRM Bot" — контакт уже в циклі нагадувань (виняток №3 за текстом чату).',
            source: 'rule',
          });
          counts.skip++;
          continue;
