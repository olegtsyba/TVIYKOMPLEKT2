...
# ефект розкладу cron.
#
# На старті регулярного розкладу (2026-08-25) свідомо залишалось у
# DRY-RUN: власниця хотіла спершу побачити реальну поведінку скрипта й
# Telegram-сповіщення (включно з окремим 🔔⚠️ алертом на "Фідбек відмова")
# на регулярній основі. Після кількох діб стабільного DRY-RUN
# CHECK_NOTIFICATIONS_LIVE=true увімкнено в .env з 2026-08-26 — відтоді
# цей крон РЕАЛЬНО дублює автоповідомлення клієнтам в Instagram Direct.
set -uo pipefail
cd "$(dirname "$0")" || exit 1
